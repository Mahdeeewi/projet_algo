...
#include <cmath>
using namespace std;
vector<string> motsSelectionnes(100); // vecteur de mots Selectionnes a partir du dictionnaire
int x = 0,
...
void remplirMatriceN1(int n, int m, vector<string> motsSelectionnes, int &khrouj)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        // Si la ligne n'a pas de '1', on force l'ajout d'un '1' quelque part, '1' represente un mur
        if (!hasOne)
        {
    ...
    recopiematrice(n, m); // on recopie la matrice pour ne pas modifier la matrice a chaque utilisation de l'algorithme djikstra
}
...
void remplirMatriceN3(int n, int m, vector<string> motsSelectionnes, int &khrouj)
{
    ...
    for (int rep = 0; rep < motsSelectionnes.size(); rep++)
    {
        ...
        while (c < k)
        {
            ...
            // Vérifier si la case est disponible, si la case est remplie avec un caractere, on chereche une autre case libre
            ...
            Matric1[row][col] = '1';   // Marquer la case comme occupée dans la matrice de controle
            c++;
        }
    }
    for (int i = 0; i < n; i++)
